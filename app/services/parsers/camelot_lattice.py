...
def _check_ghostscript() -> bool:
    return ensure_executable_on_path("gs", "gswin64c", "gswin32c") is not None


class CamelotLatticeParser(BaseParser):
    parser_id = "camelot_lattice"
    parser_label = "Camelot Lattice"
    ...
    def parse(
        ...
        try:
            ...
            return ParseResult(
                ...
                confidence=0.0,
                rows=[],
                errors=[f"Camelot lattice failed: {exc}"],
            )
    ...
    def _parse_inner(
        ...
        for tbl_idx, table in enumerate(tables):
            ...
            acc = report.get("accuracy", 0) / 100.0
            ...
            for row_idx, raw_row in df.iloc[1:].iterrows():
                try:
                    cells = [str(c).strip() for c in raw_row.tolist()]

                    date_str = cells[col_map["date"]] if col_map["date"] < len(cells) else ""
                    merchant = cells[col_map["merchant"]] if col_map["merchant"] < len(cells) else ""
                    ...
                    if txn_date is None and date_str:
                        skipped += 1
                        errors.append(f"Table {tbl_idx + 1} row {row_idx}: bad date '{date_str}'")
                        continue
        ...
        camelot_acc = sum(accuracy_scores) / len(accuracy_scores) if accuracy_scores else 0.0
        valid = sum(1 for r in rows if is_valid_row(r))
        row_confidence = compute_confidence(valid, skipped)
        confidence = round((camelot_acc + row_confidence) / 2, 3) if rows else 0.0
