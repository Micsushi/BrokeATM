...
from app.services.parsers.camelot_lattice import _check_ghostscript, _GS_ERROR
...
class CamelotStreamParser(BaseParser):
    parser_id = "camelot_stream"
    parser_label = "Camelot Stream"
    ...
    def parse(
        ...
        try:
            ...
            return ParseResult(
                ...
                confidence=0.0,
                rows=[],
                errors=[f"Camelot stream failed: {exc}"],
            )
    ...
    def _parse_inner(
        ...
        skipped = 0
        warnings: list[str] = [
            "Whitespace-based column detection: may misalign on variable-width fonts",
        ]
        ...
        if not tables or len(tables) == 0:
            warnings.append("No stream tables detected.")
            return ParseResult(
                parser_id=self.parser_id,
                parser_label=self.parser_label,
                confidence=0.0,
